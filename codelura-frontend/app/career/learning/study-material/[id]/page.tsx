...
// // Add this after your STATIC_RATINGS
// const SERVICE_SECTIONS: ServiceSection[] = [
//   {
//     id: "resume-review",
//     title: "Resume Review Session",
//     description: "Get your resume reviewed by industry experts. Get personalized feedback on content, structure, and ATS compatibility.",
//     icon: "📄",
//     slug: "resume-review",
//     price: 499,
//     isPaid: true,
...
//     description: "Check if your resume passes ATS screening. Get detailed scoring and optimization tips for better visibility.",
//     icon: "📊",
//     slug: "ats-score",
//     price: 299,
//     isPaid: true,
...
//     description: "Create a professional, ATS-friendly resume with our AI-powered builder. Choose from 50+ templates.",
//     icon: "✍️",
//     slug: "resume-builder",
//     price: 699,
//     isPaid: true,
...
//   );
// }
// // Add this component after StarRating component
...
//     <motion.div
//       whileHover={{ y: -4 }}
//       className="relative group bg-gradient-to-br from-zinc-900/80 to-zinc-800/40 rounded-2xl border border-white/[0.07] p-5 transition-all duration-300 hover:border-amber-500/30 hover:shadow-lg hover:shadow-amber-500/5"
//     >
//       {service.popular && (
//         <div className="absolute -top-2 right-4">
//           <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-[10px] font-bold text-white uppercase tracking-wider">
//             ⭐ Popular
//           </span>
//         </div>
//       )}
...
//           <span key={idx} className="text-[10px] text-zinc-500 bg-white/5 px-2 py-0.5 rounded-full border border-white/5">
//             ✓ {feature}
//           </span>
//         ))}
//         {service.features.length > 3 && (
//           <span className="text-[10px] text-zinc-600">+{service.features.length - 3} more</span>
//         )}
//       </div>
...
//           className="px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-xs font-bold text-white hover:shadow-lg hover:shadow-amber-500/25 transition-all"
//         >
//           Get Started →
//         </button>
//       </div>
...
//     }
//   }}
//   className="group relative w-full overflow-hidden rounded-2xl border border-indigo-500/30 bg-gradient-to-r from-indigo-600/20 to-violet-600/20 py-3.5 text-sm font-semibold text-white transition-all duration-300 hover:border-indigo-400/60 hover:from-indigo-600/30 hover:to-violet-600/30 hover:shadow-[0_0_20px_rgba(124,58,237,0.25)] active:scale-[0.98]"
// >
//   <span className="relative flex items-center justify-center gap-2">
//     <span className="text-base transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:scale-110">📥</span>
//     Save to Dashboard
//   </span>
// </button>
...
//               </button>
//             </div>
//             <div className="relative mx-auto max-w-[1400px] px-4 pb-16 sm:px-6">
//               <RelatedCourses
...
// Dynamic import for heavy related courses section below the fold
// const RelatedCourses = dynamic(() => import("./RelatedCourses"), { ssr: false });
...
export default function CourseDetailPage() {
  ...
  const [totalPages, setTotalPages] = useState<number | null>(null);
  const [showSticky, setShowSticky] = useState(false);
  const [accessLevel, setAccessLevel] = useState<"preview" | "full_read" | "full_access" | "locked">("preview");
  ...
  const rating = useMemo(() => {
    if (!course) return 4.5;
    const hash = course._id.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
    return STATIC_RATINGS[Math.abs(hash) % STATIC_RATINGS.length];
  }, [course]);
  ...
  if (!course) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 text-slate-500 font-semibold">
        Course not found
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[300px_1fr_300px] lg:items-start xl:grid-cols-[320px_1fr_320px]">
          <LeftSidebar course={course} rating={rating} totalPages={totalPages} />
        ...
        {/* Related Courses */}
        {/* <div className="mt-12">
          <RelatedCourses currentId={course._id} tags={course.tags} category={course.category} limit={4} />
        </div> */}
      </div>

      {/* Native Mobile Sticky CTA */}
      <StickyMobileCTA price={course.price} isPaid={course.isPaid} show={accessLevel === "locked" && showSticky} />
    </div>
  );
}