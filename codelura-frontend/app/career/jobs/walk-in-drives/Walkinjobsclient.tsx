...
const TABS = [
  { key: "walk-in", label: "🚶 Walk-in", color: "#5c1a8a" },
  ...
  return (
    ...
    >
      ...
      <div className="job-badges">
        <span className="jb badge-walkin">Walk-in</span>
...
function EmptyState({ tab }: { tab: TabKey }) {
  const map: Record<TabKey, { icon: string; title: string; sub: string }> = {
    "walk-in": { icon: "🚶", title: "No walk-in drives yet",  sub: "We'll list them here as soon as they're announced." },
    expired:   { icon: "⏰", title: "No expired listings",    sub: "All current walk-in listings are still active." },
  };
...
/* ─────────────────────────────────────────
   MAIN PAGE CLIENT (WALK-IN ONLY)
───────────────────────────────────────── */
export default function WalkInJobsClient({ jobs }: { jobs: Job[] }) {
  const [activeTab, setActiveTab] = useState<TabKey>("walk-in");
  const [pages, setPages] = useState<Record<TabKey, number>>({
    "walk-in": 1, expired: 1,
  });

  // 🔒 Hard filter: sirf "walk-in" type jobs, koi bhi internship/full-time/off-campus yahan nahi aayegi
  const walkInOnly = jobs.filter((j) => j.type === "walk-in");

  const activeWalkIn  = walkInOnly.filter((j) => !j.isExpired);
  const expiredWalkIn = walkInOnly.filter((j) => j.isExpired);

  const tabData: Record<TabKey, Job[]> = {
    "walk-in": activeWalkIn,
    expired:   expiredWalkIn,
  };

  const counts: Record<TabKey, number> = {
    "walk-in": activeWalkIn.length,
    expired:   expiredWalkIn.length,
  };
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .jobs-hero::before {
          ...
          inset: 0;
          background:
            radial-gradient(ellipse 70% 60% at 80% 50%, #6b21a822 0%, transparent 70%),
            radial-gradient(ellipse 50% 80% at 10% 80%, #c084fc18 0%, transparent 60%);
          pointer-events: none;
        }
        ...
        .hero-stats { display: flex; gap: 36px; padding-top: 32px; border-top: 1px solid #2e1e38; flex-wrap: wrap; }
        ...
        .tab-btn.active { color: var(--ink); border-bottom-color: var(--tab-color, #6b21a8); }
        .tab-count {
          font-size: 10px; font-weight: 700; padding: 1px 7px;
          border-radius: 20px; background: var(--paper-2); color: var(--muted);
          transition: background 0.18s, color 0.18s;
        }
        .tab-btn.active .tab-count { background: var(--tab-color, #6b21a8); color: #fff; }
        ...
        .badge-walkin { background: #f3e6f9; color: #5c1a8a; }
        ...
        .job-card:hover .view-btn { background: #55187f; transform: translateY(-1px); }
      ...
      <div className="jobs-root">
        ...
        <header className="jobs-hero">
          <div className="hero-inner">
            ...
            <h1 className="hero-title">Walk-in <em>Drives</em><br />Happening Now</h1>
            <p className="hero-sub">
              Walk-in interview drives from top companies — curated by the Codelura
              team so you can show up prepared, no online application needed.
            </p>
            <div className="hero-stats">
              {[
                { val: activeWalkIn.length,  lbl: "Active Drives" },
                { val: expiredWalkIn.length, lbl: "Expired"       },
              ].map(({ val, lbl }) => (
                <div key={lbl}>
        ...
        <main className="jobs-body">
          <div className="section-heading">
            <h2>
              {activeTab === "walk-in" && <><em style={{ color: "#5c1a8a" }}>Walk-in</em> Drives</>}
              {activeTab === "expired" && <><em style={{ color: "#8a1a1a" }}>Expired</em> Walk-in Listings</>}
            </h2>
            {allForTab.length > 0 && (
