  ...
  return (
    ...
    >
      ...
      <div className="job-badges">
        <span className="jb badge-fulltime">Full-Time</span>
...
function EmptyState({ tab }: { tab: TabKey }) {
  const map: Record<TabKey, { icon: string; title: string; sub: string }> = {
    ...
    expired:     { icon: "⏰", title: "No expired listings",     sub: "All current full-time listings are still active." },
  };
...
/* ─────────────────────────────────────────
   MAIN PAGE CLIENT (FULL-TIME ONLY)
───────────────────────────────────────── */
export default function FullTimeJobsClient({ jobs }: { jobs: Job[] }) {
  const [activeTab, setActiveTab] = useState<TabKey>("full-time");
  const [pages, setPages] = useState<Record<TabKey, number>>({
    "full-time": 1, expired: 1,
  });

  // 🔒 Hard filter: sirf "full-time" type jobs, koi bhi internship/contract/part-time yahan nahi aayegi
  const fullTimeOnly = jobs.filter((j) => j.type === "full-time");

  const activeFullTime  = fullTimeOnly.filter((j) => !j.isExpired);
  const expiredFullTime = fullTimeOnly.filter((j) => j.isExpired);

  const tabData: Record<TabKey, Job[]> = {
    "full-time": activeFullTime,
    expired:     expiredFullTime,
  };

  const counts: Record<TabKey, number> = {
    "full-time": activeFullTime.length,
    expired:     expiredFullTime.length,
  };
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .jobs-hero::before {
          ...
          inset: 0;
          background:
            radial-gradient(ellipse 70% 60% at 80% 50%, #1a7a4a22 0%, transparent 70%),
            radial-gradient(ellipse 50% 80% at 10% 80%, #7ec8a018 0%, transparent 60%);
          pointer-events: none;
        }
        ...
        .hero-stats { display: flex; gap: 36px; padding-top: 32px; border-top: 1px solid #223226; flex-wrap: wrap; }
        ...
        .tab-btn.active { color: var(--ink); border-bottom-color: var(--tab-color, #1a7a4a); }
        .tab-count {
          font-size: 10px; font-weight: 700; padding: 1px 7px;
          border-radius: 20px; background: var(--paper-2); color: var(--muted);
          transition: background 0.18s, color 0.18s;
        }
        .tab-btn.active .tab-count { background: var(--tab-color, #1a7a4a); color: #fff; }
        ...
        .featured-badge {
          ...
          background: linear-gradient(90deg, #1a7a4a, #7ec8a0);
        ...
        .job-card:hover .view-btn { background: #145a37; transform: translateY(-1px); }
      ...
      <div className="jobs-root">
        ...
        <header className="jobs-hero">
          <div className="hero-inner">
            ...
            <h1 className="hero-title">Full-Time <em>Careers</em><br />Await You</h1>
            <p className="hero-sub">
              Only full-time roles from top companies — curated by the Codelura team
              so you can apply directly. No internships, no noise.
            </p>
            <div className="hero-stats">
              {[
                { val: activeFullTime.length,  lbl: "Active Full-Time" },
                { val: expiredFullTime.length, lbl: "Expired"          },
              ].map(({ val, lbl }) => (
                <div key={lbl}>
        ...
        <main className="jobs-body">
          <div className="section-heading">
            <h2>
              ...
              {activeTab === "expired"   && <><em style={{ color: "#8a1a1a" }}>Expired</em> Full-Time Listings</>}
            </h2>
            {allForTab.length > 0 && (
