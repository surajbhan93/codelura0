...
//         "/api/hackathons/ai/plagiarism-check",
//         {
//           ...form,
...
//             <p>
//               <strong>Level:</strong> {result.plagiarism_level}
//             </p>
...
//             <p>
//               <strong>Flags:</strong> {result.flags.join(", ") || "None"}
//             </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-gray-900">AI Plagiarism Checker</h1>
          <p className="text-sm text-gray-500 mt-1">
            Select a submission to auto-fill, then run plagiarism check
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border p-5 shadow-sm">
          ...
          {fetchingSubmissions ? (
            ...
            <select
              className="w-full border rounded-lg p-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-rose-300"
              value={selectedId}
          ...
          {selectedSub && (
            <div className="mt-3 flex items-center gap-3 p-3 bg-rose-50 rounded-lg border border-rose-100">
              <div className="w-8 h-8 rounded-full bg-rose-200 text-rose-700 font-bold text-sm flex items-center justify-center">
        ...
        <div className="bg-white rounded-xl border p-5 shadow-sm">
          ...
          <div className="space-y-3">
            <input
              ...
              placeholder="Submission ID"
              className="w-full border rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-200 bg-gray-50"
              onChange={handleChange}
            />
            <input
              name="projectTitle"
              ...
              placeholder="Project Title"
              className="w-full border rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-rose-200"
              onChange={handleChange}
            />
            <textarea
              name="projectDescription"
        ...
        <div className="bg-white rounded-xl border p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold text-gray-800">Run Plagiarism Check</h2>
          </div>

          <button
            onClick={handleSubmit}
            disabled={loading || !form.projectTitle}
            className="w-full bg-rose-600 hover:bg-rose-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold p-3 rounded-lg transition text-sm"
          >
            {loading ? (
          ...
          {result && (
            <div className="mt-5 space-y-3">
              ...
              {/* Level + Score */}
              <div className={`p-4 rounded-lg border ${LEVEL_COLORS[result.plagiarism_level]}`}>
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-wide opacity-60">Plagiarism Level</div>
                    <div className="text-2xl font-bold">{result.plagiarism_level}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-semibold uppercase tracking-wide opacity-60">Similarity Score</div>
                ...
                <div className="w-full bg-white bg-opacity-60 rounded-full h-2.5">
                  <div
                    className={`h-2.5 rounded-full transition-all duration-700 ${SCORE_BAR_COLORS[result.plagiarism_level]}`}
