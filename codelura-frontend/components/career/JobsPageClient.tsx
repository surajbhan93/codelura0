...
const TABS = [
  ...
  { key: "off-campus", label: "🏫 Off Campus", color: "#0891b2" },
  { key: "walk-in", label: "🚶 Walk-In", color: "#dc2626" },
  { key: "codelura", label: "🚀 Codelura", color: "#7c3aed" },
...
const TYPE_BADGE = {
  ...
  "off-campus": "badge-offcampus",
  "walk-in": "badge-walkin",
  codelura: "badge-codelura",
};
...
function EmptyState({ tab }: { tab: TabKey }) {
  const map: Record<TabKey, { icon: string; title: string; sub: string }> = {
    ...
    "off-campus": {
  icon: "🏫",
  title: "No Off Campus Drives",
  sub: "New off-campus opportunities will appear here.",
},

"walk-in": {
  icon: "🚶",
  title: "No Walk-In Drives",
  sub: "Walk-in hiring events will appear here.",
},

codelura: {
  icon: "🚀",
  title: "No Codelura Openings",
  sub: "Codelura hiring will be listed here.",
},
...
export default function JobsPageClient({ jobs }: { jobs: Job[] }) {
 ...
 const offCampusJobs = activeJobs.filter(
  (j) => j.type === "off-campus"
);

const walkInJobs = activeJobs.filter(
  (j) => j.type === "walk-in"
);

const codeluraJobs = activeJobs.filter(
  (j) => j.type === "codelura"
);
  ...
  "off-campus": offCampusJobs,
  "walk-in": walkInJobs,
  codelura: codeluraJobs,
  expired: expiredJobs,
  };
  ...
  "off-campus": offCampusJobs.length,
  "walk-in": walkInJobs.length,
  codelura: codeluraJobs.length,
        ...
        <main className="jobs-body">
          <div className="section-heading">
            <h2>
              ...
              <>
                <em style={{ color: "#0891b2" }}>Off Campus</em> Drives
              </>
            )}

            {activeTab === "walk-in" && (
              <>
                <em style={{ color: "#dc2626" }}>Walk-In</em> Drives
              </>
            )}

            {activeTab === "codelura" && (
              <>
                <em style={{ color: "#7c3aed" }}>Codelura</em> Hiring
              </>
            )}
            </h2>
            {allForTab.length > 0 && (
